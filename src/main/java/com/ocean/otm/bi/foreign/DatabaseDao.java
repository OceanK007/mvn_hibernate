...
public class DatabaseDao
{
	public static void main(String[] args)
	{
		...
		// Here the owner table is address table, so we need person_id first to save address object.
		// You can persist person first then persist address. Or
		// Hibernate will auto persist the person when you save address object.
		...
		//############ Codes start - Retrieving object using HQL with Join ############//
		{
			...
			Query query = session3.createQuery("SELECT DISTINCT(p) FROM com.ocean.otm.bi.foreign.Person p INNER JOIN FETCH p.addresses WHERE p.id = :id");

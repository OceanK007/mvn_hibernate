...
public class DatabaseDao
{
	public static void main(String[] args)
	{
		...
		//############ Codes start - Retrieving object using .get() method ############//
		{
			...
			Address a = (Address) session2.get(Address.class, 1L);
			System.out.println("Address detail: "+a.getAddressDetail());
			System.out.println("Person name: "+a.getPerson().getName());
			// In lazy loading, session needs to be opened as it'll hit database again to retrieve person. 
			session2.close();
		}
		...
		//############ Codes start - Retrieving object using HQL with Join ############//
		{
			...
			// We are using package name along with Person class because there are multiple Address classes
			Query query = session3.createQuery("FROM com.ocean.oto.bi.foreign.Address a INNER JOIN FETCH a.person WHERE a.id = :id");
		...
		//############ Codes start - Retrieving object using Native SQL with Join ############//
		{
			...
			Query query = session3.createSQLQuery("SELECT p.*,a.* FROM oto_bi_foreign_person p INNER JOIN oto_bi_foreign_address a ON p.address_id = a.id WHERE p.id = :id")
			...
			for(Object[] row: records)
			{
				...
				System.out.println("Address Detail: "+add.getAddressDetail());
